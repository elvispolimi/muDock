...
Generates paper-quality PDF plots from the CSV produced by profiler_macro_cpu.py.
Input : macro_results_cpu_{dataset}.csv
Output: macro_01_latency_cpu_{dataset}.pdf
        macro_02_throughput_cpu_{dataset}.pdf
        macro_03_speedup_heatmap_cpu_{dataset}.pdf
        macro_04_statistics_cpu_{dataset}.pdf / .csv
"""
...
def _backend_colors(backend: str, n: int):
    if backend == "CPP":
        return cm.Greens(np.linspace(0.40, 0.88, n))
    else: # Alpaka CPU
        return cm.Purples(np.linspace(0.40, 0.88, n))
...
def plot_latency(df: pd.DataFrame, ds_name: str) -> str:
    ...
    for backend, grp in mean_df.groupby("Backend"):
        ...
        for idx, pop in enumerate(populations):
            ...
            ax.plot(
                ...
                linestyle="-" if backend == "CPP" else "--",
            )

    ax.set_title(f"CPU Latency Scaling — {ds_name}")
    ...
    out = f"macro_01_latency_cpu_{ds_name}.pdf"
    fig.savefig(out)
    plt.close(fig)
    return out
...
def plot_throughput(df: pd.DataFrame, ds_name: str) -> str:
    ...
    ax.set_title(f"CPU Throughput Scaling — {ds_name}")
    ax.set_xlabel("Number of Generations")
    ...
    try:
        cpp_peak    = mean_df[(mean_df["Backend"]=="CPP")        & (mean_df["Population"]==populations[-1])]["Throughput (Evals/s)"].max()
        alpaka_peak = mean_df[(mean_df["Backend"]=="Alpaka CPU") & (mean_df["Population"]==populations[-1])]["Throughput (Evals/s)"].max()
        gap_pct = (alpaka_peak / cpp_peak - 1) * 100
        sign    = "+" if gap_pct >= 0 else ""
        color   = "#27AE60" if gap_pct >= 0 else "#C0392B"
        ax.annotate(
            f"Gap @ Pop={populations[-1]}: Alpaka {sign}{gap_pct:.1f}% vs CPP",
    ...
    out = f"macro_02_throughput_cpu_{ds_name}.pdf"
    fig.savefig(out)
    plt.close(fig)
    return out
...
def plot_speedup_heatmap(df: pd.DataFrame, ds_name: str) -> str | None:
    mean_df = df.groupby(["Backend", "Population", "Generations"])["Throughput (Evals/s)"].mean().reset_index()
    df_cpp    = mean_df[mean_df["Backend"] == "CPP"].set_index(["Population", "Generations"])
    df_alpaka = mean_df[mean_df["Backend"] == "Alpaka CPU"].set_index(["Population", "Generations"])

    if df_cpp.empty or df_alpaka.empty:
        warn("Cannot generate speedup heatmap — missing one backend.")
        return None

    common = df_cpp.index.intersection(df_alpaka.index)
    if common.empty:
        warn("No common (Population, Generation) pairs between backends.")
        return None

    speedup = (df_alpaka.loc[common, "Throughput (Evals/s)"] /
               df_cpp.loc[common, "Throughput (Evals/s)"]).reset_index()
    ...
    sns.heatmap(
        ...
        cbar_kws={"label": "Speedup Alpaka CPU / CPP  (1.0 = parity)"},
        ax=ax,
    )
    ax.set_title(f"CPU Relative Speedup: Alpaka vs CPP — {ds_name}")
    ax.set_xlabel("Generations")
    ax.set_ylabel("Population")
    ax.invert_yaxis()

    # Add a 1.0 baseline annotation
    ax.annotate("< 1.0 = CPP faster   |   > 1.0 = Alpaka faster",
                xy=(0.5, -0.08), xycoords="axes fraction",
                ha="center", fontsize=9, color="#555")

    out = f"macro_03_speedup_heatmap_cpu_{ds_name}.pdf"
    fig.savefig(out)
    plt.close(fig)
    return out
...
def plot_statistics_table(df: pd.DataFrame, ds_name: str) -> tuple[str, str]:
    ...
    csv_out = f"macro_04_statistics_cpu_{ds_name}.csv"
    ...
    for (row, col), cell in table.get_celld().items():
        if row == 0:
            ...
            base = "#EBF5FB" if backend == "CPP" else "#F4ECF7"
            cell.set_facecolor(base if row % 2 == 1 else "white")
        cell.set_edgecolor("#D5D8DC")

    ax.set_title(
        f"CPU Statistical Summary: Throughput (Evals/s) — {ds_name}",
        pad=16, fontweight="bold", fontsize=12, loc="left",
    )

    pdf_out = f"macro_04_statistics_cpu_{ds_name}.pdf"
...
def generate_macro_plots() -> None:
    banner("MACRO CPU PLOTTER  —  muDock  |  Paper-Quality PDF Generation")
