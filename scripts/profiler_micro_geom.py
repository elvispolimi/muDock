"""
profiler_micro_geom.py
Micro-profiling of the geom_transform (apply) kernel via NVIDIA Nsight Systems.
Produces: micro_geom_transform.pdf
"""
...
KERNEL_FILTER = r"apply|geom"
KERNEL_LABEL  = "geom_transform"
OUTPUT_PDF    = "micro_geom_transform.pdf"
...
# ─── MAIN ────────────────────────────────────────────────────────────────────
def main():
    banner(f"MICRO PROFILING — geom_transform kernel  |  Pop:{POPULATION}  Gen:{GENERATIONS}")
    ...
    fig.suptitle(
        f"Micro-Profiling: geom_transform  —  Pop:{POPULATION}  Gen:{GENERATIONS}",
