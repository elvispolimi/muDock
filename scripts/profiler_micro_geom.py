...
def run_geom_profiling():
    gpu_frames = []

    print("="*60)
    print(" MICRO PROFILING: GEOM_TRANSFORM FOCUS (NSIGHT SYSTEMS)")
    ...
    for name, bin_path, use_flag in BACKENDS:
        ...
        report_base = f"nsys_geom_{safe_name}"
        sqlite_file = f"{report_base}.sqlite"

        print(f"\n[+] Executing nsys profile for {name}...")
        
        # Aggiungiamo --observer 1 per non inondare l'I/O (come da benchmark stress load)
        cmd_profile = [
            "nsys", "profile",
        ...
        if Path(csv_kern).exists():
            ...
            # Filtriamo SOLO il kernel apply/geom_transform
            df_geom = df_k[df_k["Name"].str.contains("apply|geom", case=False, na=False)].copy()
            
            if df_geom.empty:
                print(f"[!] No geom_transform kernels found for {name}")
                continue
                
            df_geom["Kernel"] = "geom_transform"
            ...
            print(f"  -> {name} geom_transform Total Time: {sum_k['Total Time (ms)'].iloc[0]:.2f} ms")
            print(f"  -> {name} geom_transform Avg Time:   {sum_k['Avg Time (us)'].iloc[0]:.2f} us")
            
            gpu_frames.append(sum_k)

    if gpu_frames:
        print("\n[+] Generating Geom Transform Plot...")
        df_gpu = pd.concat(gpu_frames, ignore_index=True)
        
        fig, axes = plt.subplots(1, 2, figsize=(12, 6))
        fig.suptitle(f"Micro-Vision Analysis: GEOM_TRANSFORM (Pop:{POPULATION}, Gen:{GENERATIONS})", fontweight='bold', fontsize=16)
        ...
        plt.tight_layout()
        out_name = f"focus_geom_transform_perf.pdf"
