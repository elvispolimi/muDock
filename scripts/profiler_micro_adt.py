...
def run_adt_profiling():
    gpu_frames = []

    print("="*60)
    print(" MICRO PROFILING: ADT_SCORE FOCUS (NSIGHT SYSTEMS)")
    ...
    for name, bin_path, use_flag in BACKENDS:
        ...
        if Path(csv_kern).exists():
            ...
            # Filtriamo SOLO i kernel dell'energia
            df_geom = df_k[df_k["Name"].str.contains("calc_energy|adt_score", case=False, na=False)].copy()
            
            if df_geom.empty:
                print(f"[!] No adt_score kernels found for {name}")
                continue
                
            df_geom["Kernel"] = "adt_score"
            ...
            print(f"  -> {name} adt_score Total Time: {sum_k['Total Time (ms)'].iloc[0]:.2f} ms")
            print(f"  -> {name} adt_score Avg Time:   {sum_k['Avg Time (us)'].iloc[0]:.2f} us")
            
            gpu_frames.append(sum_k)

    if gpu_frames:
        print("\n[+] Generating Adt Score Plot...")
        df_gpu = pd.concat(gpu_frames, ignore_index=True)
        
        fig, axes = plt.subplots(1, 2, figsize=(12, 6))
        fig.suptitle(f"Micro-Vision Analysis: ADT_SCORE (Pop:{POPULATION}, Gen:{GENERATIONS})", fontweight='bold', fontsize=16)
        ...
        plt.tight_layout()
        out_name = f"focus_adt_score_perf.pdf"
