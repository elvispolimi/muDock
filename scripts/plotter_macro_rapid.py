...
PALETTE     = {"CUDA": "#E74C3C", "Alpaka": "#3498DB"}
PALETTE_LIGHT = {"CUDA": "#F1948A", "Alpaka": "#85C1E9"}
...
def plot_rapid(ds_name: str, df: pd.DataFrame) -> str:
    ...
    colors = [PALETTE.get(b, "#888") for b in backends]
    ...
    # Speedup / delta annotation between the two bars
    if "CUDA" in backends and "Alpaka" in backends:
        cuda_mean   = stats.loc[stats["Backend"] == "CUDA",   "Mean"].values[0]
        alpaka_mean = stats.loc[stats["Backend"] == "Alpaka", "Mean"].values[0]
        pct         = (alpaka_mean / cuda_mean - 1) * 100
        sign        = "+" if pct >= 0 else ""
        color       = "#27AE60" if pct >= 0 else "#C0392B"
        arrow_y     = max(cuda_mean, alpaka_mean) * 0.5
        ax.annotate(
            "", xy=(1, arrow_y), xytext=(0, arrow_y),
            arrowprops=dict(arrowstyle="<->", color=color, lw=1.5),
        )
        ax.text(0.5, arrow_y * 1.04, f"{sign}{pct:.1f}%",
    ...
    for i, backend in enumerate(backends):
        color       = PALETTE.get(backend, "#888")
        color_light = PALETTE_LIGHT.get(backend, "#ccc")
...
# ─── MAIN ────────────────────────────────────────────────────────────────────
def main() -> None:
    banner("RAPID PLOTTER  —  muDock  |  CUDA vs Alpaka Quick Comparison")

    for ds_name in DATASETS:
        csv_file = f"macro_results_rapid_{ds_name}.csv"
