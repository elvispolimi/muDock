...
namespace mudock::stream_engine {
  ...
  public:
    std::optional<lig_batch> add(const lig_ptr& ligand) {
  ...
  public:
    ...
    void operator()(lig_ptr ligand, batcher_mfn::output_ports_type& out) const {
      if (auto batch = state_.add(std::move(ligand))) {
        std::get<0>(out).try_put(std::make_shared<lig_batch>(std::move(*batch)));
      }
    }
  };
