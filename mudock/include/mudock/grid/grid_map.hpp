  ...
  public:
    ...
    grid_map(const dynamic_molecule& receptor)
        : grid(get_grid_points_dim(get_grid_minimum(receptor), get_grid_maximum(receptor))),
          ...
          center{(maximum.x - minimum.x) / fp_type{2} + minimum.x,
                 (maximum.y - minimum.y) / fp_type{2} + minimum.y,
                 (maximum.z - minimum.z) / fp_type{2} + minimum.z},
    ...
    grid_map(const index3D _sizes, const point3D _center)
        : grid(_sizes),
          minimum(-std::ceil(_sizes.size_x() / fp_type{2}) * grid_spacing + _center.x,
                  -std::ceil(_sizes.size_y() / fp_type{2}) * grid_spacing + _center.y,
                  -std::ceil(_sizes.size_z() / fp_type{2}) * grid_spacing + _center.z),
          maximum(std::ceil(_sizes.size_x() / fp_type{2}) * grid_spacing + _center.x,
                  std::ceil(_sizes.size_y() / fp_type{2}) * grid_spacing + _center.y,
                  std::ceil(_sizes.size_z() / fp_type{2}) * grid_spacing + _center.z),
          center{_center},
          minimum_coord({minimum.x + grid_spacing, minimum.y + grid_spacing, minimum.z + grid_spacing}),
