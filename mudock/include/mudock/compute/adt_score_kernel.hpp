...
namespace mudock {
  ...
  template<typename queue_type>
    requires std::derived_from<queue_type, queue>
  struct adt_gradient_kernel {
    static constexpr char adt_region_name[] = "adt_gradient_kernel";
    adt_gradient_kernel(const int scores_per_ligand_,
                        const int batch_ligands_,
                        const int batch_atoms_,
                        const int *__restrict__ num_atoms_b_,
                        const int *__restrict__ num_rotamers_b_,
    ...
    adt_gradient_kernel(const adt_gradient_kernel &)            = default;
    adt_gradient_kernel(adt_gradient_kernel &&)                 = default;
    adt_gradient_kernel &operator=(const adt_gradient_kernel &) = delete;
    adt_gradient_kernel &operator=(adt_gradient_kernel &&)      = delete;

    ~adt_gradient_kernel() = default;
