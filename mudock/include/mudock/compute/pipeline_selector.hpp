...
namespace mudock {
  ...
  template<>
  struct pipeline_selector<search_algorithm::NONE, scoring_function::XSCORE> {
    using type = mudock::x_score_pipeline;
  };

  template<search_algorithm search, scoring_function scoring>
  using pipeline_selector_t = typename pipeline_selector<search, scoring>::type;

  // dispatch_selected_pipeline walks every search x scoring combination at compile time, so it has to
  // recognise the combinations that were left unspecialised above instead of failing to instantiate.
  template<search_algorithm search, scoring_function scoring, typename = void>
  inline constexpr bool has_pipeline_v = false;

  template<search_algorithm search, scoring_function scoring>
  inline constexpr bool
      has_pipeline_v<search, scoring, std::void_t<typename pipeline_selector<search, scoring>::type>> = true;
  ...
  template<>
  struct scoring_function_format<scoring_function::XSCORE>
      : scoring_function_format_base<scoring_function_format<scoring_function::XSCORE>> {
        ...
        [&](const auto search_index) {
          ...
          constexpr_switch<0, num_scoring_functions(), 1>(
              [&](const auto scoring_index) {
                ...
                if constexpr (!has_pipeline_v<selected_search, selected_scoring>) {
                  throw std::runtime_error(std::string{"Scoring function '"} +
                                           std::string{to_string(selected_scoring)} +
                                           "' does not support the search algorithm '" +
                                           std::string{to_string(selected_search)} + "'");
                } else {
