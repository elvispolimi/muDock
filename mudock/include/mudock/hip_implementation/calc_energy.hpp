...
namespace mudock {
  ...
  __device__ fp_type calc_ddd_Mehler_Solmajer_hip(fp_type distance) {
  ...
  // TODO check math functions if they are from HIP
  __device__ fp_type calc_intra_energy(const fp_type* ligand_x,
    ...
    for (int nonbond_list = threadIdx.x; nonbond_list < ligand_num_nonbonds; nonbond_list += blockDim.x) {
      ...
      const fp_type r_dielectric = fp_type{1} / (distance * calc_ddd_Mehler_Solmajer_hip(distance));
