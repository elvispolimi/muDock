...
namespace mudock {
  ...
  class language_scratchpad_interface {
  public:
    // The scratchpad requires the device context in which move in and out the data
    language_scratchpad_interface(const device_context<l_t>& d_c): d_context(d_c){};
    virtual ~language_scratchpad_interface() {}

    // A scratchpad cannot be copied or moved
    language_scratchpad_interface(const language_scratchpad_interface&)            = delete;
    language_scratchpad_interface(language_scratchpad_interface&&)                 = delete;
    language_scratchpad_interface& operator=(const language_scratchpad_interface&) = delete;
    language_scratchpad_interface& operator=(language_scratchpad_interface&&)      = delete;

    // Given a span of input molecule on the host, copy ON the device memory pointed by device_context
    virtual void copy_in(const std::span<std::unique_ptr<static_molecule>>) = 0;
    // Given a span of output molecule on the host, copy FROM the device memory pointed by device_context
    virtual void copy_out(std::span<std::unique_ptr<static_molecule>>) = 0;
  ...
  // NOTE: this enables a useful mechanism of implementation selection based on the language_types
  template<language_types l_t>
  class language_scratchpad_impl {};
} // namespace mudock
