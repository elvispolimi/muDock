...
namespace mudock {
  ...
  inline fp_type calc_ddd_Mehler_Solmajer_cuda(fp_type distance) {
    ...
    fp_type epsilon = A + B / (fp_type{1} + rk * sycl::exp(lambda_B * distance));
  ...
  fp_type calc_intra_energy(const fp_type* ligand_x,
    ...
    for (int nonbond_list = it.get_local_id(0); nonbond_list < ligand_num_nonbonds;
      ...
      const fp_type distance_two = sycl::pow(sycl::fabs(ligand_x[a1] - ligand_x[a2]), fp_type{2}) +
                                   sycl::pow(sycl::fabs(ligand_y[a1] - ligand_y[a2]), fp_type{2}) +
                                   sycl::pow(sycl::fabs(ligand_z[a1] - ligand_z[a2]), fp_type{2});
      ...
      if (distance_two_clamp < nbc2) {
        ...
        if ((hbond_i == 1 || hbond_i == 2) && hbond_j > 2) {
          ...
          epsij = sycl::sqrt(epsii_i * epsii_j);
        }
        if (xA != xB) {
          const fp_type tmp = epsij / (xA - xB);
          const fp_type cA  = tmp * sycl::pow(Rij, xA) * xB;
          const fp_type cB  = tmp * sycl::pow(Rij, xB) * xA;

          const fp_type rA = sycl::pow(distance, static_cast<fp_type>(xA));
          const fp_type rB = sycl::pow(distance, static_cast<fp_type>(xB));

          e_vdW_Hb = sycl::fmin(EINTCLAMP, (cA / rA - cB / rB));

          if (r_smooth > 0) {
            const fp_type rlow  = distance - r_smooth / 2;
            const fp_type rhigh = distance + r_smooth / 2;
            fp_type energy_smooth{100000};
            for (int j = sycl::fmax(fp_type{0}, sycl::fmin(rlow * A_DIV, fp_type{NEINT - 1}));
                 j <= sycl::fmin(fp_type{NEINT - 1}, sycl::fmin(rhigh * A_DIV, fp_type{NEINT - 1}));
                 ++j)
              energy_smooth = sycl::fmin(energy_smooth, e_vdW_Hb);
