...
namespace mudock {
  ...
  virtual_screen_omp::virtual_screen_omp(const knobs k,
    ...
    atom_texs.wrappers_pointer.alloc(num_autodock_grid_types());
    atom_texs.wrappers.reserve(num_autodock_grid_types());
    for (int index{0}; index < num_autodock_grid_types(); ++index) {
