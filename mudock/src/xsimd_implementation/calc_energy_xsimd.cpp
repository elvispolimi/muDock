...
namespace mudock {
  ...
  template<>
  fp_type calc_energy<cpu_vectorization::XSIMD>(const fp_type* __restrict__ ligand_x,
    ...
    for (std::size_t index = 0; index < num_atom_loops * simd_size; index += simd_size) {
      ...
      if (xsimd::any(inside)) {
        ...
        // Load map offsets
        auto atom_map_offsets = xsimd::load_unaligned(map_ligand_offsets + index);
    ...
    if (n_torsions > 0) {
      ...
      for (size_t i = 0; i < num_non_bond_loops * simd_size; i += simd_size) {
        ...
        const auto diff_x = batch_type::gather(ligand_x, a1) - batch_type::gather(ligand_x, a2);
        const auto diff_y = batch_type::gather(ligand_y, a1) - batch_type::gather(ligand_y, a2);
        const auto diff_z = batch_type::gather(ligand_z, a1) - batch_type::gather(ligand_z, a2);
        ...
        const auto epsilon =
            ms_B_vec *
                (batch_type(1.0) / (batch_type(1.0) + ms_rk_vec * xsimd::exp(ms_lambda_B_vec * distance))) +
            ms_A_vec;

        const auto r_dielectric = batch_type(1.0) / (distance * epsilon);

        const auto charge_a1 = batch_type::gather(ligand_charge, a1);
        const auto charge_a2 = batch_type::gather(ligand_charge, a2);
        ...
        const auto ligand_vol_a2_v    = batch_type::gather(ligand_vol, a2);
        const auto ligand_solpar_a1_v = batch_type::gather(ligand_solpar, a1);
        const auto ligand_solpar_a2_v = batch_type::gather(ligand_solpar, a2);

        const auto nb_desolv = ligand_vol_a2_v * (ligand_solpar_a1_v + qsolpar_vec * xsimd::abs(charge_a1)) +
                               ligand_vol_a1_v * (ligand_solpar_a2_v + qsolpar_vec * xsimd::abs(charge_a2));

        const auto e_desolv = coeff_desolv_vec *
