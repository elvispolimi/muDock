...
namespace mudock {
  ...
  void init_texture_memory(const grid_map &map, hipTextureObject_t &tex_obj, const hipStream_t &stream) {
    // Create 3D HIP array for the texture
    hipArray *d_array;
    ...
    copyParams.srcPtr =
        make_hipPitchedPtr((void *) map.data(), extent.width * sizeof(fp_type), extent.width, extent.height);
    ...
    // tex_desc.filterMode     = hipFilterModeLinear; // Enable linear interpolation
  ...
  virtual_screen_hip::virtual_screen_hip(const knobs k,
    ...
    std::size_t index{0};
    atom_texs.alloc(num_autodock_grids());
