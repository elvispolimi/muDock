...
namespace mudock {
  ...
  void setup_constant_memory(const point3D& minimum_coord,
    ...
    MUDOCK_CHECK(
        hipMemcpyToSymbol(&inv_spacing_const, &inv_grid_spacing, sizeof(fp_type), 0, hipMemcpyHostToDevice));
  }
...
#pragma unroll
  ...
  __device__ int get_selection_distribution(hiprandState& state, const int* population_number) {
    return random_gen_hip<int>(state, 0, *population_number - 1);
  };

  __device__ fp_type get_init_change_distribution(hiprandState& state) {
    return random_gen_hip<fp_type>(state, -45, 45);
  }
  __device__ fp_type get_mutation_change_distribution(hiprandState& state) {
    return random_gen_hip<fp_type>(state, -10, 10);
  };
  __device__ fp_type get_mutation_coin_distribution(hiprandState& state) {
    return random_gen_hip<fp_type>(state, 0, 1);
  };
  __device__ int get_crossover_distribution(hiprandState& state, const int* num_rotamers) {
    return random_gen_hip<int>(state, 0, 6 + *num_rotamers);
  };

  __device__ int tournament_selection_hip(hiprandState& state,
  ...
  __global__ void evaluate_fitness(const int num_generations,
    ...
    hiprandState& l_state                = (state[threadId]);

    // Shared memory
    extern __shared__ fp_type shared_data[];
    fp_type* s_chromosome_scores = shared_data;
    // printf("I'm %d %d atoms %d\n", threadId, ligand_id, num_atoms);
      ...
      for (int chromosome_index = 0; chromosome_index < chromosome_number; ++chromosome_index) {
        ...
        for (int atom_index = threadIdx.x; atom_index < num_atoms; atom_index += blockDim.x) {
          ...
          if (coord_tex[0] < map_min_const[0] || coord_tex[0] > map_max_const[0] ||
            ...
            elect_total_trilinear +=
                trilinear_interpolation_hip(coord_tex, electro_texture) * l_ligand_charge[atom_index];
            dmap_total_trilinear +=
                trilinear_interpolation_hip(coord_tex, desolv_texture) * fabsf(l_ligand_charge[atom_index]);
            emap_total_trilinear +=
                trilinear_interpolation_hip(coord_tex, atom_textures[l_atom_tex_indexes[atom_index]]);
          }
        }
